...
- **Pieza reto "tuerca" (N)**: una octava pieza de 3×3 con un hueco vacío en el centro (forma de anillo). Ese hueco no colisiona, no se dibuja y no se fija al tablero al bloquearla, así que la fila que pasa por él no se puede limpiar hasta que otra pieza rellene esa celda concreta. Aparece con la misma probabilidad que el resto (1 de cada 8).
...
Aporta el aspecto visual con estética _dark / retro arcade_ por defecto: fondo oscuro, tipografía monoespaciada para los marcadores y _backdrop blur_ en los overlays. Los colores están definidos como variables CSS en `:root` (modo oscuro) y se sobrescriben bajo el selector `body[data-theme="light"]` para el modo claro.
